...
import { setNewestShop, setShopCategory } from "redux/slices/shopFilter";
  ...
  return (
    <div className={cls.wrapper}>
      <Link href="/" className={cls.item} onClick={filterByCategory}>
        <span className={cls.text}>{'Alll'}</span>
      </Link>
      <Link href="/" className={cls.item} onClick={filterNewestShop}>
        <span className={cls.text}>{t("new")}</span>
      </Link>
      {data.map((item) => (
        <Link
          href="/"
